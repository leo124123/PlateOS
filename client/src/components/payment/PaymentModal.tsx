...
import { X, CreditCard, DollarSign, ArrowRight, CheckCircle2, Receipt, Percent, ShieldCheck } from 'lucide-react';
...
export const PaymentModal: React.FC<PaymentModalProps> = ({ table, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200 select-none">
      <div className="w-full max-w-lg glass-panel rounded-3xl border border-slate-700/80 p-6 overflow-hidden shadow-2xl bg-slate-900/95 text-white flex flex-col gap-5">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-black tracking-tight text-white">Cobro — Mesa #{table.number}</h3>
        ...
        {isSuccess ? (
          <div className="py-12 flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mb-4 animate-bounce border border-emerald-500/40 shadow-lg shadow-emerald-500/20">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2 text-xs">
              ...
              <div className="pt-2.5 border-t border-slate-800 flex justify-between text-base font-black text-white">
                <span>Total a Cobrar:</span>
                <span className="text-emerald-400 text-lg font-black">${finalTotal.toFixed(2)}</span>
              </div>
            </div>

            {/* Tip Percentage Selection */}
            <div>
              <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider mb-2 flex items-center gap-1">
            ...
            <div>
              <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider mb-2 block">
                Método de Pago
              </label>
              <div className="grid grid-cols-3 gap-2.5">
                ...
                >
                  <CreditCard className="w-5 h-5 text-emerald-400" /> Tarjeta
                </button>
                <button
                ...
                >
                  <DollarSign className="w-5 h-5 text-emerald-400" /> Efectivo
                </button>
                <button
                ...
                >
                  <Receipt className="w-5 h-5 text-emerald-400" /> Transferencia
                </button>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-xl shadow-emerald-500/20 transition-all disabled:opacity-50"
            >
              {isProcessing ? (
                <span>Procesando Cobro...</span>
              ) : (
                <>
                  Confirmar Cobro y Liberar Mesa <ArrowRight className="w-4 h-4" />
                </>
              )}
            </button>
          </div>
